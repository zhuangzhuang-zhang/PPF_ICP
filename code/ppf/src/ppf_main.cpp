...
const double camera_cx = 311.832;//325.5//319.5 310.95 310.95
const double camera_cy = 241.665;//253.5//239.5 234.74 234.74
const double camera_fx = 620.845;//518.0//570.3422(openni2) 615.377
const double camera_fy = 620.845;//519.0//570.3422(openni2) 615.377
ppf::PPF6DDetector detector(0.06, 10.0, 12.0, 0.05); //0.05, 9, 12, 0.03
...
bool obj_s(obj_srv::obj_6d::Request &req,obj_srv::obj_6d::Response &res)
{
    if(req.start)
    {
        ...
        for(int i=0;i<results.size();i++)
        {
            ...
            /*cv::Matx44d pose_ttt = cv::Matx44d::eye();
            pose_ttt(1,1) = 0;pose_ttt(2,1) = 1;pose_ttt(1,2) = -1;pose_ttt(2,2) = 0;
            results[i]->updatePose(pose_ttt);*/
            /*cv::Vec4d q_ttt = cv::Vec4d::all(0);
            q_ttt[0] = 0.707;q_ttt[1] = 0.707;
            cv::Vec3d tt = cv::Vec3d::all(0);
            results[i]->updatePoseQuat(q_ttt, tt);*/
...
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh;
    ros::ServiceClient client = nh.serviceClient<obj6d_msgs::rgbd>("realsense_server");
